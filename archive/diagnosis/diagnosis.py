...
def get_top_picks():
            ...
            "data": [
                {
                    "code": str,
                    "name": str,
                    "overall_score": float,  # 使用诊断引擎的综合评分
                    "reason": str,
                    "strategy_name": str,
                    "price": float
                },
    ...
    try:
        # 从今日精选获取候选股票
        from src.web.cache_manager import get_cache
        ...
        logger.info(f"开始诊断 {len(picks)} 只候选股票...")
        
        diagnosed_picks = []
        for pick in picks[:20]:  # 只诊断前20只，避免太慢
            try:
                code = pick.get('code', '')
                # 调用诊断引擎
                report = diagnosis_engine.diagnose_stock(code)
        ...
        logger.info(f"Top 10 推荐完成，共 {len(top_10)} 只股票")
        ...
        logger.error(f"获取 Top 10 推荐失败: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
